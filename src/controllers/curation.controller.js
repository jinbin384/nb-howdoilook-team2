// /src/controllers/curation.controller.js (순수 함수 기반 ES Module)
...
import {
  ...
  NotFoundError,
  ValidationError,
} from "../utils/CustomError.js"; // Named Import로 가정

// 헬퍼 함수: 페이지네이션 쿼리 파싱 (Named Export)
...
// ------------------------------------
// 1. 큐레이팅 등록 (POST) (Named Export)
// ------------------------------------
export const createCurationController = async (req, res, next) => {
  try {
...
export const deleteCurationController = async (req, res, next) => {
  try {
    ...
    // 서비스 호출
    const message = await curationService.deleteCuration(
      curationId,
      inputPassword
    );
...
// ------------------------------------
// 4. 큐레이팅 목록 조회 (GET) (Named Export)
// ------------------------------------
export const getCurationListController = async (req, res, next) => {
  try {
    // 경로, 쿼리 파라미터에서 값 추출
    const { styleId } = req.params;
